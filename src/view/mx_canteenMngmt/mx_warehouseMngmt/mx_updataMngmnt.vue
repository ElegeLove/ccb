...
const defaultCheckedList = ["肉禽蛋类", "水果饮品"];
export default {
    data() {
        return {
    ...
    mounted() {
        ...
        this.$route.query.data.greats = this.$route.query.data.greats.split(",")
        let greats = []
        this.$route.query.data.greats.forEach(item => {
            item = parseInt(item)
            greats.push(item)
        });
        this.form.setFieldsValue({
            canteenId: data.canteen_id,
            warehouseName: data.title,
            address: data.location,
            administrators: data.lead,
            telephone: data.lead_phone,
            storageClass: greats
        });
        this.getCanteen();
        this.getStoreClass();
        ...
        handleSubmit(e) {
            ...
            this.form.validateFields((err, values) => {
                if (!err) {
                    ...
                    let data = {
                        url: "api/school/store/" + this.storeId,
                        method: 'put',
                        data: {
